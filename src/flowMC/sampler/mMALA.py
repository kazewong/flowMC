...
def make_mMALA_kernel(logpdf, d_logpdf, d2_logpdf, dt):
    def mMALA_kernel(rng_key, position, log_prob):
        ...
        d2_current = jax.lax.cond(jnp.any(jnp.isnan(d2_current)), lambda:jnp.eye(n_dim), lambda:d2_current)
        ...
        proposal += jnp.dot(dt_current_sqrt,jax.random.normal(key1, shape=position.shape))
        proposal_log_prob = logpdf(proposal)
        d2_proposal = d2_logpdf(proposal)
        d2_proposal = jax.lax.cond(jnp.any(jnp.isnan(d2_proposal)), lambda:jnp.eye(n_dim), lambda:d2_proposal)
        d2_proposal = jnp.linalg.inv(d2_proposal)

        ratio = proposal_log_prob - logpdf(position)
        ratio -= multivariate_normal.logpdf(position, proposal+jnp.dot(d2_proposal*dt*dt,d_logpdf(proposal))/2,d2_proposal*dt*dt)
        ratio += multivariate_normal.logpdf(proposal, position+jnp.dot(d2_current*dt*dt,d_log_current)/2,d2_current*dt*dt)
...
def make_mMALA_update(logpdf, d_logpdf, d2_logpdf, dt):
    ...
    def mMALA_update(i,state):
        ...
        new_position, new_log_prob, do_accept = mMALA_kernel(key, positions[i-1],
                                                                log_prob[i-1])
