...
class GaussianRandomWalk(LocalSamplerBase):
    ...
    def make_kernel(self, return_aux = False) -> Callable:
        def rw_kernel(rng_key, position, log_prob):
            key1, key2 = jax.random.split(rng_key)
            move_proposal = jax.random.normal(key1, shape=position.shape) * self.step_size
    ...
    def make_update(self) -> Callable:
        ...
        def rw_update(i, state):
            ...
            new_position, new_log_p, do_accept = rw_kernel(key, positions[i-1], log_p[i-1])
    ...
    def make_sampler(self) -> Callable:
        ...
        rw_update = jax.vmap(rw_update, in_axes = (None, (0, 0, 0, 0)), out_axes=(0, 0, 0, 0))
        lp = jax.vmap(lp)
