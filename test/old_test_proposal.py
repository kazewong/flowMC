...
def sample_nf(model, param, rng_key, n_sample):
    ...
    samples = model.apply(
        {"params": param}, subkey, n_sample, param, method=model.sample
    )
...
def sampling_loop(rng_keys_nf, rng_keys_mcmc, model, state, initial_position):
    rng_keys_mcmc, positions, log_prob = run_mcmc(
        rng_keys_mcmc, n_samples, dual_moon_pe, initial_position
    )
    ...
    rng_keys_nf, nf_chain, log_prob, log_prob_nf = nf_metropolis_sampler(
        rng_keys_nf, nf_samples, model, state.params, dual_moon_pe, positions[:, -1]
    )
...
chains = []
for i in range(3):
    rng_keys_nf, rng_keys_mcmc, state, positions = sampling_loop(
        rng_keys_nf, rng_keys_mcmc, model, state, last_step
    )
...
proposal_position = model.apply(
    {"params": params},
    subkey,
    initial_position.shape[1] * nf_samples,
    params,
    method=model.sample,
)[0]

log_pdf_nf_proposal = model.apply(
    {"params": params}, proposal_position, method=model.log_prob
)
log_pdf_nf_initial = model.apply(
    {"params": params}, initial_position.T, method=model.log_prob
)
...
proposal_position = proposal_position.reshape(
    nf_samples, initial_position.shape[1], initial_position.shape[0]
)
...
nf_metropolis_kernel(
    subkeys,
    proposal_position[0],
    initial_position.T,
    log_pdf_proposal[0],
    log_pdf_nf_proposal[0],
    log_pdf_initial,
    log_pdf_nf_initial,
)
