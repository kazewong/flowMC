...
class TestLogPDF:
    ...
    def test_value_and_grad(self):
        logpdf = LogPDF(self.posterior, [Variable("x_"+str(i), True) for i in range(self.n_dims)])
        input = jnp.arange(self.n_dims)
        values, grads = jax.value_and_grad(logpdf)(input)
        assert values == self.posterior(input, jnp.ones(self.n_dims))

    def test_vmap(self):
        def call_logPDF(logpdf, x):
            return logpdf(x)
        logpdf = LogPDF(self.posterior, [Variable("x_"+str(i), True) for i in range(self.n_dims)])
        logpdfs = [logpdf, logpdf]
        inputs = jnp.arange(self.n_dims).reshape(-1, self.n_dims)
        values = jax.vmap(call_logPDF, in_axes=(0, None))(logpdfs, inputs)
    ...
    def test_resource(self):
        ...
        initial_pdf = logpdf(initial_position, data)
        resources = {
        ...
        stepper = TakeSerialSteps("logpdf", "MALA", ["test_position", "test_log_prob", "test_acceptance"], 1)
